...
def validate_prepared_data(data, *, path: Path, split: str) -> None:
    missing = [field for field in REQUIRED_DATA_FIELDS if not hasattr(data, field)]
    if missing:
...
class PreparedGraphDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        validate_prepared_data(data, path=path, split=self.split)
...
def build_dataloaders(
    ...
    datasets = {
        split: PreparedGraphDataset(metadata, split=split, edge_mode=config.edge_mode)
        for split in CANONICAL_SPLITS
    }
    loaders = {
        split: DataLoader(
...
def build_model(metadata: PreparedMetadata, config: BaselineConfig) -> GraphSAGEStateClassifier:
...
def _use_cuda_amp(device: torch.device, config: BaselineConfig) -> bool:
...
def build_train_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train the baseline GNN from a prepared artifact cache")
    parser.add_argument(
        "--config",
        type=str,
        default=str(DEFAULT_BASELINE_CONFIG_PATH),
        help="Path to the baseline training JSON config",
    )
    parser.add_argument(
        "--prepared-root",
        type=str,
